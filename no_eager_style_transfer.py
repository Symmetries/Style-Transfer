...
#image_transformation_network.add(BatchNormalization())
image_transformation_network.add(Conv2DTranspose(filters=8, kernel_size=3, padding='same', activation='relu'))
image_transformation_network.add(BatchNormalization())
...
def image_transformation_loss(y_true, y_pred):
    ...
    return compute_loss(pre_trained_model, (1e-3, 1e-1), y_pred, gram_style_features, content_features)[0]
...
for i in range(50):
    ...
    imsave('even_faster_net_outputs/output_{:03d}.png'.format(i), out)
    
    tf.keras.models.save_model(
        image_transformation_network,
        'even_faster_models/network_{:03d}.h5'.format(i),
        overwrite=True,
